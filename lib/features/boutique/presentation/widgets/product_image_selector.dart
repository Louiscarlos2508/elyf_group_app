...
class _ProductImageSelectorState extends State<ProductImageSelector> {
  ...
  Future<void> _pickImage(ImageSource source) async {
    ...
    try {
      ...
      if (errorCode == 'photo_access_denied' ||
        ...
        errorMessage =
            'Permission refusée. Veuillez autoriser l\'accès à la caméra/galerie dans les paramètres.';
      } else if (errorCode == 'photo_access_denied_permanently') {
        errorMessage =
            'Permission refusée définitivement. Veuillez l\'activer dans les paramètres de l\'application.';
        ...
        errorMessage =
            'Erreur d\'ouverture de la caméra. Sur cet appareil, veuillez utiliser la galerie photo à la place.';
      } else if (errorMessageLower.contains('channel') ||
          errorMessageLower.contains('connection') ||
          errorCode.contains('channel')) {
        errorMessage =
            'Erreur de connexion avec le plugin. Veuillez redémarrer complètement l\'application (Hot Restart ou rebuild).';
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text('Image du produit', style: theme.textTheme.labelLarge),
