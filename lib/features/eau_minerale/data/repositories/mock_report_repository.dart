...
class MockReportRepository implements ReportRepository {
  ...
  Future<ReportData> fetchReportData(ReportPeriod period) async {
    ...
    final collections = periodSales.fold(
      0,
    ...
    final monthlyTotal = periodMonthlyPayments.fold(
      0,
      (sum, p) => sum + p.amount,
    );
    final productionTotal = periodProductionPayments.fold(
      0,
  ...
  Future<ProductionReportData> fetchProductionReport(
    ...
    final periodSessions = sessions.where((session) {
      ...
      return (sessionDate.isAfter(start.subtract(const Duration(days: 1))) ||
              sessionDate.isAtSameMomentAs(start)) &&
          (sessionDate.isBefore(end.add(const Duration(days: 1))) ||
              sessionDate.isAtSameMomentAs(end));
    }).toList();

    final totalQuantity = periodSessions.fold(
      0,
  ...
  Future<SalaryReportData> fetchSalaryReport(ReportPeriod period) async {
    ...
    final periodMonthlyPayments = monthlyPayments.where((p) {
      ...
      return (payDate.isAfter(start.subtract(const Duration(days: 1))) ||
              payDate.isAtSameMomentAs(start)) &&
          (payDate.isBefore(end.add(const Duration(days: 1))) ||
              payDate.isAtSameMomentAs(end));
    }).toList();
    ...
    final totalMonthlySalaries = periodMonthlyPayments.fold(
      0,
      (sum, p) => sum + p.amount,
    );
    final totalProductionPayments = periodProductionPayments.fold(
      0,
      (sum, p) => sum + p.totalAmount,
    );
