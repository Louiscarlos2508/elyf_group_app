...
class GazDashboardKpiGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Calculs des KPIs
    final totalStock = cylinders.fold<int>(0, (sum, c) => sum + c.stock);
    ...
    final monthExpensesTotal = monthExpenses.fold<double>(0, (sum, e) => sum + e.amount);

    // Bénéfice du mois
    final monthProfit = monthRevenue - monthExpensesTotal;

    // Ventes détail vs gros
    final retailSales = monthSales.where((s) => s.saleType == SaleType.retail).length;
    final wholesaleSales = monthSales.where((s) => s.saleType == SaleType.wholesale).length;
