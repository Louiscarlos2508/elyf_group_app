...
class GazStockCalculationService {
  ...
  static List<CylinderStock> filterEmptyStocks(List<CylinderStock> stocks) {
    return stocks.where((s) => s.status == CylinderStatus.emptyAtStore).toList();
  }
  ...
  static List<CylinderStock> filterIssueStocks(List<CylinderStock> stocks) {
    return stocks.where((s) =>
        ...
        s.status == CylinderStatus.leakInTransit).toList();
  }
  ...
  static StockMetrics calculateStockMetrics({
    ...
    final relevantStocks = isPOS && targetEnterpriseId != null
        ? stocks.where((s) => s.enterpriseId == targetEnterpriseId).toList()
        : stocks;
    ...
    final inTransitStocks = relevantStocks.where((s) => s.status == CylinderStatus.emptyInTransit).toList();
    ...
    for (final s in inTransitStocks) {
       final enterprise = pointsOfSale.where((p) => p.id == s.enterpriseId).firstOrNull;
       final name = enterprise?.name ?? (s.enterpriseId == targetEnterpriseId ? 'Local (Tournées)' : 'Autre');
    ...
    if (transfers != null) {
      ...
      final relevantTransfers = isPOS && targetEnterpriseId != null
          ? transfers.where((t) => t.status == StockTransferStatus.shipped && t.toEnterpriseId == targetEnterpriseId)
          : transfers.where((t) => t.status == StockTransferStatus.shipped && (t.toEnterpriseId == targetEnterpriseId || t.fromEnterpriseId == targetEnterpriseId));
    ...
    for (final cylinder in cylinders) {
      ...
      if (settings != null && settings.nominalStocks.isNotEmpty) {
        ...
        if (isMother && nominal > 0) {
          final full = fullByWeight[weight] ?? 0;
          final issues = issueByWeight[weight] ?? 0;
          
          theoreticalEmptyByWeight[weight] = (nominal - full - issues - totalInTransit).clamp(0, nominal);
        }
      }
    }
    ...
    for (final weight in weightsToShow) {
      final full = fullByWeight[weight] ?? 0;
      final empty = theoreticalEmptyByWeight.isNotEmpty 
          ? (theoreticalEmptyByWeight[weight] ?? 0) 
          : (emptyByWeight[weight] ?? 0);
      ...
      final leaks = issueStocks
          .where((s) => s.weight == weight && (s.status == CylinderStatus.leak || s.status == CylinderStatus.leakInTransit))
          .fold<int>(0, (sum, s) => sum + s.quantity);
  ...
  static PointOfSaleStockMetrics calculatePosStockMetrics({
    ...
    final posStocks = allStocks.where((s) => s.enterpriseId == enterpriseId && (siteId == null || s.siteId == siteId)).toList();
    ...
    final Map<int, int> realInTransitByWeight = Map.from(emptyInTransitByWeight);
    if (transfers != null) {
      final incomingShipped = transfers
          .where((t) => t.toEnterpriseId == enterpriseId && t.status == StockTransferStatus.shipped)
          .toList();
      for (final transfer in incomingShipped) {
        for (final item in transfer.items) {
          realInTransitByWeight[item.weight] = (realInTransitByWeight[item.weight] ?? 0) + item.quantity;
        }
      }
    }

    final totalInTransit = realInTransitByWeight.values.fold<int>(0, (sum, v) => sum + v);
  ...
  static String formatStockByWeightSummary(Map<int, int> stockByWeight, List<int> weights) {
    if (weights.isEmpty) {
