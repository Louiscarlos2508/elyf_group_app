        ...
        for (final entry in remoteIdUpdates.entries) {
          try {
            await driftService.records.updateRemoteId(
              ...
              enterpriseId: operations.first.enterpriseId,
            );
          } catch (e, stackTrace) {
            final appException = ErrorHandler.instance.handleError(e, stackTrace);
            AppLogger.warning(
              'Error updating remoteId after batch create: ${appException.message}',
              name: 'offline.firebase.batch',
              error: e,
              stackTrace: stackTrace,
            );
      ...
      if (serverUpdatedAt != null &&
        ...
        if (driftService != null) {
          try {
            ...
            final appException = ErrorHandler.instance.handleError(e, stackTrace);
            AppLogger.warning(
              'Error updating local with server version: ${appException.message}',
              name: 'offline.firebase.batch',
              error: e,
              stackTrace: stackTrace,
            );
